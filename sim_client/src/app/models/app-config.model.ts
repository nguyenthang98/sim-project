...
export class AppConfig {
	...
	initStage(containerId) {
		...
		const _diffScaleY =
			_containerBoundingClient.height / this.mainConfig.height;
		const _newScale = Math.min(_diffScaleX, _diffScaleY) * 0.8;
		this.stage.scaleX(_newScale);
		this.stage.scaleY(_newScale);
		this.stage.x(
			(_containerBoundingClient.width - this.mainConfig.width * _newScale) / 2
		);
		this.stage.y(
			(_containerBoundingClient.height - this.mainConfig.height * _newScale) / 2
		);
	...
	changeStageScale(event) {
		...
		const _containerBoundingClient = this.stage
			.container()
